...
function replaceAddExistingAssistantCommand(selectedEntityTypeName, selectedControl, firstPrimaryItemId) {
    if (selectedControl.getRelationship().name == "procure_incident_contact_assistants_mm") {
        // Custom Case -> Contact N:N - filters to show only contacts with contact types surgeon and Assistant
        var options = {
            ...
            filters: [{ 
                ...
                filterXml: "<filter type='and'><filter type='or'><condition attribute='procure_contacttype' operator='in'><value>692820001</value><value>692820002</value></condition></filter></filter>"
            }]
        };

        lookupAddExistingRecords("procure_incident_contact_assistants_mm", "incident","contact", firstPrimaryItemId, selectedControl, options);
    }
    else {
